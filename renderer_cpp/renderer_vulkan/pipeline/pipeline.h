...
public:
    ...
    // パイプラインを生成。例外を投げず std::expected でエラーハンドリング
...
[[nodiscard]] inline std::expected<std::vector<char>, EngineError> read_file(const std::string& filename) noexcept {
...
// シェーダーモジュール生成ヘルパー
[[nodiscard]] inline std::expected<VkShaderModule, EngineError> create_shader_module(
    VkDevice device,
...
// データ指向の観点から、状態を保持する純粋なデータコンテナとして定義
struct GraphicsPipeline {
    ...
    [[nodiscard]] static std::expected<GraphicsPipeline, EngineError> create(
        ...
        // 4. パイプラインレイアウト(Push Constantsなど) の設定 (PipelineLayoutSetup を統合)

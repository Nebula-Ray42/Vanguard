...
typedef uint32_t SlangSizeT;
    ...
    */
    ...
    //! Use to tests if a result was failure. Never use result != SLANG_OK to tests for failure, as
    //! there may be successful codes != SLANG_OK.
#define SLANG_FAILED(status) ((status) < 0)
    //! Use to tests if a result succeeded. Never use result == SLANG_OK to tests for success, as will
...
struct IGlobalSession : public ISlangUnknown
{
    ...
    */
    ...
    @param target The compilation target to tests

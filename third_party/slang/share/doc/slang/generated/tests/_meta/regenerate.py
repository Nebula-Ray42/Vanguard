#!/usr/bin/env python3
"""Driver for the agentic tests suite under docs/generated/tests/.

Tracks which tests bundles are stale relative to their source documentation
and the source paths they declare in manifest.yaml, and lints generated
tests bundles for structural conformance.
...
Coverage data is never fed into a tests-writing prompt as line-level
...
# Required META keys on every generated .slang tests file. The block lives
...
_ALLOWED_INTENTS = (
    "functional",  # canonical smoke tests, one per claim
...
_ALLOWED_OOS_REASONS = (
    # Test-harness alternatives — testable, just not via slang-tests //TEST
    "needs-unit-tests",  # observable in C++ but not via slangc CLI
    "needs-multi-file-tests",  # cross-module / multi-translation-unit setup
    "needs-cli-tests",  # CLI invocation / env / exit code / flag mapping
...
_CITATION_KINDS = {"spec", "sibling-tests", "older-slangc", "doc"}
...
def validate_finding(path: Path, finding: dict) -> list[LintIssue]:
    ...
    if isinstance(exp, dict):
        ...
        # For path-style citations (doc, sibling-tests, spec), require the
        # path to resolve on disk. older-slangc citations are git refs,
        # not paths, so we don't check them here.
        if ck in ("doc", "sibling-tests", "spec") and isinstance(citation, str):
    ...
    for lineno, raw in enumerate(path.read_text().splitlines(), start=1):
        ...
        if s.startswith("#"):
            if link_re.search(s):
                active_link = True
            continue
        # Non-comment, non-blank → must be a real path. slang-tests matches
        # expected-failure entries against the full tests name, which for
...
def render_finding_md(finding: dict) -> str:
    ...
    lines.append("")
    lines.append("Surfaced by agentic tests generation.")
    lines.append("")
...
def parse_test_meta(text: str) -> dict[str, str]:
    """Parse leading //META lines from a .slang tests file.
    ...
    for line in text.splitlines():
        ...
        if s.startswith("//"):
            if out:
                # //META block ended; treat subsequent // as tests body
                break
            continue
...
def _lint_test_file(spec: BundleSpec, tf: Path) -> list[LintIssue]:
    ...
    # Every tests file must contain at least one //TEST or //DIAGNOSTIC_TEST
    # directive — otherwise slang-tests will silently skip it.
    if "//TEST" not in text and "//DIAGNOSTIC_TEST" not in text:
        issues.append(
            LintIssue(
                rel,
                "error",
                "no //TEST or //DIAGNOSTIC_TEST directive; slang-tests would skip this file",
            )
        )
    ...
    #     substring text and are what slang-tests's "Suggested
...
def cmd_index(args: argparse.Namespace) -> int:
    ...
    out.append(
        "- Each `.slang` tests file starts with a `//META` block declaring `doc_ref`,"
...
# --------------------------------------------------------------------------
# verify — run slang-tests against bundles and report pass/fail/ignored
# --------------------------------------------------------------------------


def _find_slang_test() -> Path | None:
    """Locate a slang-tests binary the agent can run locally.
    ...
    repo_root = REPO_ROOT
    candidates = [
        repo_root / "build" / "Release" / "bin" / "slang-tests",
        repo_root / "build" / "Release" / "bin" / "slang-tests.exe",
        repo_root / "build" / "RelWithDebInfo" / "bin" / "slang-tests",
        repo_root / "build" / "RelWithDebInfo" / "bin" / "slang-tests.exe",
        repo_root / "build" / "Debug" / "bin" / "slang-tests",
        repo_root / "build" / "Debug" / "bin" / "slang-tests.exe",
    ]
    for p in candidates:
...
def _missing_required_tools(test_file_meta: dict[str, str]) -> list[str]:
    ...
    Empty list when the tests has no requires-tool or all tools are
    present.
    """
    ...
    for tok in [t.strip() for t in raw.split(",") if t.strip()]:
        ...
        if bin_name is None:
            ...
            # Be conservative: treat as missing so the tests gets ignored
...
def _load_expected_failures() -> set[str]:
    ...
    comments and ignored; blank lines too. Returns a set of tests paths
    (workspace-relative).
    """
    path = (
        REPO_ROOT
...
def cmd_verify(args: argparse.Namespace) -> int:
    """Run slang-tests against the named bundles (or all of them) and
    ...
    from PATH are filtered out of the slang-tests invocation
    ...
    if slang_test is None:
        print("ERROR: slang-tests not found in any of the standard build dirs.")
        print("Build it first, e.g.:")
        print("    cmake --build --preset release --target slang-tests")
    ...
    # tool that's not on PATH. slang-tests's `-exclude-prefix` filters
    ...
    cmd: list[str] = [str(slang_test), "-tests-dir", "docs/generated/tests"]
    if args.server_count > 1:
        cmd.extend(["-use-tests-server", "-server-count", str(args.server_count)])
    ...
    # slang-tests exit code is non-zero when any tests fails; that's
    ...
    for line in out.splitlines():
        if line.startswith("FAILED tests:"):
            # Format: FAILED tests: 'docs/generated/tests/.../foo.slang'
            m = re.match(r"^FAILED tests:\s*'([^']+)'", line)
            if m:
                failed_all.append(m.group(1))
        elif line.startswith("passed tests:"):
            passed += 1
        elif line.startswith("ignored tests:"):
            ignored += 1
    ...
    # slang-tests prints a summary like "98% of tests passed (3413/3482), 47 tests ignored"
    summary_line = ""
    for line in out.splitlines():
    ...
    # Tests skipped by `-exclude-prefix` don't appear in slang-tests
    ...
    print(f"  ignored:       {ignored}    (backend not available in slang-tests — CI validates)")
    if skipped_by_tool:
        print(
    ...
    if failed:
        ...
        print()
        print(
            "Fix every FAILED tests before commit. Most common causes:"
        )
        print(
        ...
        print(
            "    actual-output and either fix the CHECK or rewrite the tests)."
        )
        print(
...
def _collect_test_doc_refs() -> dict[str, list[str]]:
    ...
    workspace-relative tests paths that cite it.
...
def cmd_lang_ref_coverage(args: argparse.Namespace) -> int:
    """Report how much of docs/language-reference/ has at least one tests
    ...
    # `test_refs` maps `path#anchor` strings to tests paths. Bucket
    ...
    if args.mapping:
        ...
        # For each tests that cites a lang-ref anchor, look up the
        # bundle it lives in and that bundle's source_doc. If the
        # bundle's source_doc is a design-doc, record an edge.
        manifest = load_manifest()
        # Build path → source_doc lookup for the bundle a tests lives in.
        ...
        if not edges:
            print("  (no edges yet — no tests cites lang-ref from inside a"
                  " design-doc bundle.)")
        else:
            for (lang_ref, design_doc), n in sorted(edges.items()):
                print(f"  {lang_ref}\n    ↔ {design_doc}  ({n} tests(s))")
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="regenerate.py",
        description="Driver for the agentic tests suite under docs/generated/tests/.",
    )
    ...
    p_verify = sub.add_parser(
        "verify",
        help=(
            "run slang-tests against bundle(s); report pass/FAILED/ignored."
    ...
    p_verify.add_argument(
        ...
        default=4,
        help=(
            "slang-tests server count (default 4; pass 1 to disable"
            " tests-server and get more readable single-tests errors)"
        ),
    )
    p_verify.add_argument(
        "--verbose",
    ...
    p_lrc = sub.add_parser(
        ...
        help=(
            ...
            " have at least one tests anchored to them"
        ),
    )
    p_lrc.add_argument(
        "--list-uncovered",
